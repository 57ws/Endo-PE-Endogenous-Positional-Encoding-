...
### 2. 逻辑掩码位置编码
基于Transformer解码器的上三角逻辑掩码（阻止当前时步受未来信息影响），通过带掩码的自注意力生成位置信息，实现类RNN的时序位置标注：
```python
...
### 4. 融合方案
- **sin_gru**：GRU位置编码与正弦编码串联融合
- **sin_Logical**：逻辑掩码位置编码与正弦编码串联融合


## 实验结果
实验在4个任务（两点距离计算、CoNLL2003命名实体识别、IMDB文本分类、EEG脑电信号分类）中验证，以下为关键任务按F1分数（或核心指标）降序排列的结果：

### 1. 两点距离计算（核心指标：MSE/R²，按R²降序）

在长度100的序列中标记两个点，计算两点之间距离
...
> GRU组表现优秀，无位置编码优于其他，表明该任务对位置信息依赖低。
